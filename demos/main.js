...
var mytiles = [{'x': 19300, 'y': 24626.0, 'z': 16},
 ...
 // {'x': 19308.0, 'y': 24622.0, 'z': 16}];
    ...
    // console.log("min:", min);
    // console.log("max:", max);
    // export each vbo and make a downloadable blob from it
        var saveTiles = function() {
            for (var t in mytiles) {
                var mt = mytiles[t];
                var coords = mt.x+"/"+mt.y+"/"+mt.z;
                ...
                var verts = Array.prototype.slice.call(new Float32Array(scene.tiles[coords].meshes.polygons.vertex_data));
                ...
                // use filesaver
                var blob = new Blob([verts], {type: 'text/plain'});

                saveAs(blob, 'vboexport ('+t+').txt'); // uses FileSaver.js: https://github.
            }
        }

        var counter = function() {
            count++;
            if (count == mytiles.length) {
                saveTiles();
            }
        }
        ...
        function loadTiles(callback) {
            ...
            for (var t in mytiles) {
                tile = scene._loadTile(mytiles[t], callback(count));
